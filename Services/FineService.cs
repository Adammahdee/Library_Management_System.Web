...
namespace Library_Management_System.Web.Services
{
    public class FineService : IFineService
    {
        ...
        {
            await EnsureOverdueFineAsync(transaction, DateTime.UtcNow, updateOpenLoanStatus: true);
        }
        ...
        public async Task<decimal> EnsureOverdueFineAsync(BorrowTransaction transaction, DateTime effectiveDate, bool updateOpenLoanStatus)
        {
            ...
            else if (existingUnpaidFine.Amount < computedAmount)
            {
                // Update existing unpaid fine if the overdue amount has increased
                existingUnpaidFine.Amount = computedAmount;
            }

            if (updateOpenLoanStatus && transaction.ReturnDate == null && transaction.Status != "Overdue")
            {
